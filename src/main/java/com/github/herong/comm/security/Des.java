 ...
 * 
 * @author herong
 * @createTime 2013-7-22 上午10:37:05
 * @modifier
...
public class Des implements IEncryptor {
     ...
     */
    private String strDefaultKey = null;
     ...
     */
    public static String byte2HexStr(byte ib) {
    ...
    public String encode(String str) {
        ...
        if (str.length() % 8 != 0) {
            ...
            int paddingSize = 8 - str.length() % 8;
        ...
        digestHexStr = "";
        for (int i = 0; i < i1; i++) {
            digestHexStr += byte2HexStr(byte2[i]);
        }
        // System.out.println("encodehex:" + digestHexStr);
        return digestHexStr;
    }
    ...
    public String decode(String str) {
        ...
        byteMiW = this.hexStr2Byte(str);
        i2 = this.DECRYPT(this.strDefaultKey.getBytes(), byteMingW, byteMiW,
                byteMiW.length);
    ...
    public byte[] encrypt(byte[] key, byte[] source) throws Exception {
        ...
        byte[] byte2 = new byte[3000];
        int i = ENCRYPT(key, str.getBytes(), byte2, str.getBytes().length);
        return Arrays.copyOf(byte2, i);
    }
    ...
    public byte[] decrypt(byte[] key, byte[] source) throws Exception {
        byte[] byteMingW = new byte[3000]; // 密文
        int i = this.DECRYPT(key, byteMingW, source,
                source.length);
        return Arrays.copyOf(byteMingW, i);
    }
